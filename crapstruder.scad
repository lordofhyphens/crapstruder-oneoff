...
difference()
{
  translate([0,0,4])cube_fillet([44,44,10],top=[2,2,2,2],bottom=[3,3,3,3],radius=2, center=true);
  nema17();
  nema17(holes=true);
  translate([0,0,3])cube([26,20,10],center=true);
  translate([(-41/2)+25,45/2,4])rotate([90,0,0]) cylinder(r=1.3,h=45,$fs=.2);
  translate([0,0,-5])cylinder(r=22/2,h=26);
}
translate([(-41/2)+25,45/2-.6,4])rotate([-90,90,0]) 
{
  difference() {
